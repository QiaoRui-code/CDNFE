...
"""
data = pd.read_csv('cesc.csv', sep=',', header=None, index_col=0)
...
def entropy(Plist, exp_list):
    if len(Plist) >= 2:
        ...
        for x in range(len(Plist)):
            result += -(1 / math.log(len(Plist))) * (Plist[x] * abs(exp_list[x])) * math.log(
                (Plist[x] * (abs(exp_list[x]) + es)))
    else:
...
def re(sample_test, gene_name, predict_pairs1):
    ...
    for i in range(len(gene_name)):
        ...
        if outdeed_result == []:
            ...
            outdeed_local_entropy = (len(outdeed_result) / (
                    len(outdeed_result) + len(indeed_result))) * outdeed_local_entropy
        ...
        if indeed_result == []:
            ...
            indeed_local_entropy = (len(indeed_result) / (
                    len(outdeed_result) + len(indeed_result))) * indeed_local_entrop
            local_entropy = outdeed_local_entropy + indeed_local_entropy
...
if __name__ == "__main__":
    ...
    for ii in range(len(test)):
        ...
        for i in gene_name:
            ...
            rt.append(regu)
        text_save(path0 + path_name[0][0:-4] + '_CESC_pcc0_r.txt', rt)
        f = codecs.open(path0 + path_name[0][0:-4] + '_CESC_pcc0_r.txt', mode='r', encoding='utf-8')
        line = f.readline()
        targets = []
        regulators = []
        while line:
        ...
        f.close()
        print(ii)
        filename = path0 + path_name[0][0:-4] + '_CESC_pcc0_r.txt'
        predict_pairs1 = pairs1(path0 + path_name[0][0:-4] + '_CESC_pcc0_r.txt')
        df = pd.DataFrame(predict_pairs1)
        df.to_csv('CESC_gene_causality.csv', index=False, header=False)
